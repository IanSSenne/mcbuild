...
consumer.Generic = (file, tokens, func) => {
    ...
    if (/^if\(/.test(token)) {
        ...
        func.addCommand(`scoreboard players set #MC_INT_IF ${CONFIG.internalScoreboard} 0`)
        func.addCommand(`execute store success score #MC_INT_IF ${CONFIG.internalScoreboard} if ${condition} run ${consumer.Block(file, tokens, "conditional")}`);
    } else if (/^else if\(/.test(token)) {

        const condition = token.substr(8, token.length - 9);
        func.addCommand(`execute if score #MC_INT_IF ${CONFIG.internalScoreboard} matches 0 store success score #MC_INT_IF ${CONFIG.internalScoreboard} if ${condition} run ${consumer.Block(file, tokens, "conditional")}`);
    } else if (/^else/.test(token)) {
        func.addCommand(`execute if score #MC_INT_IF ${CONFIG.internalScoreboard} matches 0 run ${consumer.Block(file, tokens, "conditional")}`);
        ...
        func.addCommand(`execute ${cond} run function ${waitFunc.getReference()}`);
        // execute if score #success value matches 0 run schedule function $block 20t
    } else {
