...
const { MCFunction, loadFunction, tickFunction, loadFile } = require("./io");
const consumer = {};
...
consumer.Entry = (file, tokens, once) => {
  ...
  while (tokens[0] && !breakout) {
    ...
    if (token.startsWith("import")) {
      ...
      loadFunction.set(file, func.getReference());
      loadFile.confirm(file);
    } else {
      tokens.shift();
      throw new CompilerError(
...
consumer.Generic = (file, tokens, func, parent, functionalparent) => {
  ...
  if (token === "<%%") {
    ...
    try {
      new Function("emit", "args", code)((command) => {
        func.addCommand(command);
      }, _token.args);
    } catch (e) {
