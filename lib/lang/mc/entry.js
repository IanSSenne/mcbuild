...
consumer.Generic = (file, tokens, func) => {
    ...
    if (/^if\s*\(/.test(token)) {
        ...
        while (regex_elseif.test(tokens[0].token)) {
            ...
            func.addCommand(`execute if score #MC_INT_IF_${thisIfId} ${CONFIG.internalScoreboard} matches 0 store success score #MC_INT_IF_${thisIfId} ${CONFIG.internalScoreboard} ${condition} run ${consumer.Block(file, tokens, "conditional")}`);
        }
...
function MC_LANG_HANDLER(file) {
    ...
    namespaceStack = [location.substr(0, location.length - 3)];
