...
consumer.Generic = (file, tokens, func, parent, functionalparent) => {
  ...
  if (/^if\s*\(/.test(token)) {
    ...
    func.addCommand(
      `scoreboard players set #if ${CONFIG.internalScoreboard} 0`
    );
    func.addCommand(
      `execute ${condition} run ${consumer.Block(
        ...
        "conditional",
        {
          append: [`scoreboard players set #if ${CONFIG.internalScoreboard} 1`],
        },
        parent,
        functionalparent
      )}`
    ...
    while (regex_elseif.test(tokens[0].token)) {
      ...
      func.addCommand(
        `execute if score #if ${
          ...
          {
            append: [
              `scoreboard players set #if ${CONFIG.internalScoreboard} 1`,
            ],
          },
          parent,
          functionalparent
