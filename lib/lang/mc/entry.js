...
consumer.Generic = (file, tokens, func) => {
    ...
    if (/^if\s*\(/.test(token)) {
        ...
        func.addCommand(`execute store success score #MC_INT_IF ${CONFIG.internalScoreboard} ${condition} run ${consumer.Block(file, tokens, "conditional")}`);
    } else if (/^else if\s*\(/.test(token)) {
