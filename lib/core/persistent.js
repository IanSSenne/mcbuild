...
const EXIT_HANDLER = (code, ...rest) => {
    if (!FLAG_IS_EXITING) {
        logger.info(`shutdown signal recieved (${`${code} ${rest.join(" ")}`.trim()})`);
        FLAG_IS_EXITING = true;
        logger.info("writing cross run data to disk");
        if (!fs.existsSync(SAVE_DIR)) fs.mkdirSync(SAVE_DIR);
        fs.writeFileSync(CROSS_RUN_PATH, bson.serialize(Array.from(DATA.entries())));
        logger.info("exiting...");
        process.exit(code);
    }
};
...
if (fs.existsSync(CROSS_RUN_PATH)) {
    DATA = new Map(Object.values(bson.deserialize(fs.readFileSync(CROSS_RUN_PATH))));
