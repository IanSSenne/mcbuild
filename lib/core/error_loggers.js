...
function user(error) {
  ...
  logger.error(
    Chalk.bold.magentaBright(
      `⠴${center(Chalk.bold.black("[User Error]"), width + 19, "⠒")}⠦`
    )
  );
  logger.error(`${Chalk.bold.magentaBright("⠇" + " ".repeat(width) + "⠸")}`);
  logger.error(
    `${Chalk.bold.magentaBright("⠇" + center(error.message, width, " ") + "⠸")}`
  );
  logger.error(
    `${Chalk.bold.magentaBright(
      "⠇" + center("line:" + error.line, width, " ") + "⠸"
  ...
  logger.error(`${Chalk.bold.magenta("⠇" + " ".repeat(width) + "⠸")}`);
  logger.error(Chalk.bold.magenta(`⠙${"⠒".repeat(width)}⠋`));
}
module.exports = { critical, compiler, user };
